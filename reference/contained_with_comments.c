...
// remove risky capabilities from the container.
int capabilities()
{
...
// mount the containers directories and pivot to that directory as root for the container.
...
// use seccomp to allow only the listed syscalls that will originate from teh container.
int syscalls()
{
...
// global var for the cgroup settings
...
// set cgroup max values (similar to cgset cmd)
...
int handle_child_uid_map (pid_t child_pid, int fd)
{
	int uid_map = 0;
	int has_userns = -1;
  // wait till child has created usernamespace
  // and update uid and gid map
  ...
  // child is waiting, so notify it to proceeed
...
// create a user namespace and update parent via socketpair
int userns(struct child_config *config)
{
	...
	int result = 0;
  // now wait for parent to process the uid_map and gid_map and respond
  ...
  // change the uid and gid for the child,
  // since the child was inherited from parent, it'll inherit all the permissions as well
  // changing the uid and gid to otherthan the parent.
...
finish_options:
  ...
  // create a socket pair, an IPC mechanism (TODO: i assume its for comms between parent and container)
	if (socketpair(AF_LOCAL, SOCK_SEQPACKET, 0, sockets)) {
		fprintf(stderr, "socketpair failed: %m\n");
		goto error;
	}
  // socketpair creates two sockets, one at index 0 is parent and 1 is for child.
  // in sockerpair terminologies
  // now we set the CLOEXEC flag on the parent socket
  // so when the child calls any of exec* functions, the socket will be closed.
	if (fcntl(sockets[0], F_SETFD, FD_CLOEXEC)) {
		fprintf(stderr, "fcntl failed: %m\n");
		goto error;
	}
  //store the child socket file descriptor in the config
	config.fd = sockets[1];
  ...
  // create a child process, but seperate clone it into new namespaces listed below
  ...
  // close the child part of socketpair
  // unsure, why its done twice
	close(sockets[1]);
	sockets[1] = 0;
	close(sockets[1]);
	sockets[1] = 0;
